...
- A real Pinterest probe then exposed the remaining browser-only weakness: `i.pinimg.com` returned a valid public JPEG without a CORS permission header. Added a same-origin Vercel importer restricted to that exact host, with HTTPS/credential/port checks, public-DNS enforcement, redirect revalidation, a 12 MB streaming ceiling, MIME and byte-signature validation, `no-store`/`nosniff` responses, and rejection of arbitrary hosts. The real 50,022-byte Pinterest image passes through the importer; 54 web tests, typecheck and production build pass. Deployment and one native public-URL mission remain the final gate.
